...
python adapt_loop.py     # 自适应闭环：观察→学律→映射→改律→不动点（含非平稳失效情形）
```
...
**闭环还能自适应（模型空间的不动点迭代）**：见 `adapt_loop.py`——观察真实轨迹→学律→写入虚拟世界→验"虚拟律=真实律"→不等则改律→重复。真实平稳时迭代收缩到**不动点**（虚拟律=真实律且不再变）；真实**非平稳**时（规律随时变）没有不动点，只能跟踪不能收敛——"直到不动点"仅在"真实平稳 + 假设类覆盖真实"时成立，否则应改为"持续跟踪 + 告警"。
...
| `adapt_loop.py` | Python 演示：自适应闭环（观察→学律→映射→改律），真实平稳时收敛到不动点、真实非平稳时只能跟踪 |
