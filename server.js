...
function secHeaders() {
  return {
    ...
    'Content-Security-Policy': "default-src 'self'; img-src 'self' data:; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-inline'; object-src 'none'; base-uri 'none'",
  };
}
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'POST' && /^\/api\/(class\/start|class\/reply|teach)$/.test(req.url.split('?')[0])) {
